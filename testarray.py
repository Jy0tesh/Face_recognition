...
for f in glob.glob(os.path.join(faces_folder_path, "*.jpg")):
    ...
    for k, d in enumerate(dets):
        ...
        array1 = np.zeros((128,1), dtype=float)

        for j in range(0,128):
            array1[j] = face_descriptor[j]

        str2 = ".txt"

        g = os.path.splitext(f)[0] + str2

        print array1                

        with open(g, 'wb') as ff:
            #np.savetxt(ff ,array1,fmt='%.18e')
            np.savetxt(ff ,array1)
        ff.close()
